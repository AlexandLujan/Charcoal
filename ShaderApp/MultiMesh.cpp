...
shared_ptr<SceneNode> CShaderAppDlg::CreateSceneMultipleMesh(string objectName, shared_ptr<CommandList> commandList)
{
	...
	for (auto& [meshName, mesh] : m->MeshList())
	{
		for (size_t i = 0; i + 1 < mesh->groupStartIndex.size(); i++)
		{
...
vector<string> MaterialColorNames = {
	"Red",
	"Green",
	"Blue",
	"Cyan",
...
std::string CShaderAppDlg::RandomColor()
{
	static std::mt19937 gen(static_cast<unsigned int>(std::time(0)));
	std::uniform_int_distribution<size_t> dist(0, MaterialColorNames.size() - 1);
	return MaterialColorNames[dist(gen)];
}

shared_ptr<MeshObject> MultiMeshObject::CreateBox(float width, float height, float depth, float rollAngle, XMFLOAT3 CenterPoint)
{
	XMMATRIX identity = DirectX::XMMatrixIdentity();
...
//	XMMATRIX worldMatrix = matTranslate * matRotateZ;
	...
	string boxColor = MaterialColorNames[dist(gen)];
	float dWidth = width / 2.0f;
	float dHeight = height / 2.0f;
	float dDepth = depth / 2.0f;
	VertexType ruf = make_pair(XMFLOAT4(dWidth, dHeight, dDepth, 1), FindColor(boxColor));
	VertexType rlf = make_pair(XMFLOAT4(dWidth, -dHeight, dDepth, 1), FindColor(boxColor));
	VertexType luf = make_pair(XMFLOAT4(-dWidth, dHeight, dDepth, 1), FindColor(boxColor));
	VertexType llf = make_pair(XMFLOAT4(-dWidth, -dHeight, dDepth, 1), FindColor(boxColor));
	VertexType rur = make_pair(XMFLOAT4(dWidth, dHeight, -dDepth, 1), FindColor(boxColor));
	VertexType rlr = make_pair(XMFLOAT4(dWidth, -dHeight, -dDepth, 1), FindColor(boxColor));
	VertexType lur = make_pair(XMFLOAT4(-dWidth, dHeight, -dDepth, 1), FindColor(boxColor));
	VertexType llr = make_pair(XMFLOAT4(-dWidth, -dHeight, -dDepth, 1), FindColor(boxColor));
	...
	shared_ptr<MeshObject> box = make_shared<MeshObject>();
